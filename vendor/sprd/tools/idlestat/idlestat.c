...
static int display_statepercent(struct report_ops *ops, void *arg, void *arg1, char *cpu, void *report_data)
{
	int i;

	/* struct cpuidle_cstates *cstates = arg1; */

	if(!strncmp(cpu, "cluster", 7)){
		struct cpu_physical *s_phy = arg;
		struct cpu_cpu *s_cpu;
		struct cpufreq_pstates *pstates;
		struct cpuidle_cstates *cstates;

		s_cpu = list_entry((s_phy->cpu_enum_head).next, typeof(*s_cpu), list_phy_enum);
		pstates = s_cpu->pstates;
		cstates = s_cpu->cstates;

		if(strncmp(cpu, "clusterS", 11)) {
			total_dur_cluster_pstates = 0;
			state_dur_cluster_freq_array = calloc(sizeof(double), pstates->max-1);
			if(!state_dur_cluster_freq_array)
				return 1;
			cluster_mhz = 0;
			cluster_opp = 0;
		}

		ops->statepercent_table_header(report_data);
		ops->statepercent_hdr_hdr(cpu, report_data);

		for (i = 1; i < pstates->max; i++) {
			...
			if(strncmp(cpu, "clusterS", 11)) {
				ops->statepercent_hdr_middle_p(p, report_data);
			} else {
				cluster_opp = state_dur_cluster_freq_array[i-1]/total_dur_cluster_pstates;
				printf("| ");
				display_factored_percent(cluster_opp, 10);
				cluster_mhz += cluster_opp * p->freq;

			}
		}

		if(strncmp(cpu, "clusterS", 11)){
			ops->statepercent_hdr_tail_p(cpu, report_data);
			...
			for (i = 0; i < cstates->cstate_max + 1; i++) {
				...
				c = &(cstates->cstate[i]);
				ops->statepercent_hdr_middle_c(c, report_data);
			}

			ops->statepercent_hdr_tail_c(cpu, report_data);
		} else {
			printf("|\n");
			ops->statepercent_table_header(report_data);
			ops->statepercent_hdr_hdr("sum", report_data);
			printf("| ");
			display_factored_freq(cluster_mhz, 10);
			printf("|\n");
			ops->statepercent_table_header(report_data);
			printf("|\n");
			total_mhz  += cluster_mhz;
			free(state_dur_cluster_freq_array);
		}
	...
	if(!strncmp(cpu, "cpu", 3)){
		...
		struct cpuidle_cstates *cstates = arg1;
		ops->statepercent_hdr_hdr(cpu, report_data);
		ops->statepercent_calc(pstates, cstates, report_data);
	}

	if(!strncmp(cpu, "totalmhz", 9)){
		ops->statepercent_table_header(report_data);
		ops->statepercent_hdr_hdr("sum_total", report_data);
		printf("| ");
		display_factored_freq(total_mhz, 10);
		printf("|\n");
	}
 ...
 */
	...
	if ((options.mode == TRACE) || args < argc) {
		...
		/*Tempary disable dynamic cpumask to actual trigger all cpu core.*/
		idlestat_disable_dynamic_cpumask(true);
		sleep(1);
		...
		if (idlestat_wake_all())
			goto err_restore_trace_options;
                idlestat_disable_dynamic_cpumask(false);
    ...
    /* calculate CPU status percent */
	if (options.display & CPU_STATE_PER) {
		/* output_handler->wakeup_table_header(report_data); */
		dump_cpu_topo_info(output_handler, report_data,
				display_statepercent, cpu_topo, 2);
		output_handler->statepercent_table_footer(report_data);
	}
